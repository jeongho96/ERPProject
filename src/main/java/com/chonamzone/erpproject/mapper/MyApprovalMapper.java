...
public interface MyApprovalMapper { 
    ...
    ApproverDTO selectApprovers(int dSeq, int loginId);//문서번호,기안자로 기안정보 찾기
    int getTotalPosts(int id); //전체 포스트 갯수 세기
    DocumentListDTO.MapperData selectByDSeq(int dSeq); //문서번호로만 문서정보 가져오기
    List<ApproverDTO> selectApproverByDSeq(int dSeq);//문서번호로 기안정보 리스트로 가져오기
    ApproverDTO selectOrder(int dSeq, int aOder);//문서번호, 기안순서로 기안정보 가져오기
